...
public class SalaryCalViewController implements Initializable{
	...
	public void salaryCal(ActionEvent event) {
		...
		if(idStaffTf.getText() == null || idStaffTf.getText() == "") {
			result = ViewSalaryCalDto.getViewSalaryCalDto().getAllSalaryCal(Date.valueOf(datePickStart.getValue()), Date.valueOf(datePickEnd.getValue()));
		}else {
			result = ViewSalaryCalDto.getViewSalaryCalDto().getSalaryBySearch(idStaffTf.getText(), Date.valueOf(datePickStart.getValue()), Date.valueOf(datePickEnd.getValue()));
		}
